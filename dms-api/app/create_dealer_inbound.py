...
def create_new_dealer(session, impel_dealer_id):
    ...
    new_dealer = Dealer(impel_dealer_id=impel_dealer_id, db_creation_date=datetime.utcnow())
    session.add(new_dealer)
    session.commit()
    return new_dealer  


def create_dealer_integration(session, impel_integration_partner_id, impel_dealer_id, dms_id, is_active=True):
    ...
    try:
        dealer = create_new_dealer(session, impel_dealer_id)
...
def create_dealer_handler(event, context):
    ...
    with DBSession() as session:
        try:
            dealer_integration = create_dealer_integration(session, impel_integration_partner_id, impel_dealer_id, dms_id)
