...
def upload_unified_json(entries: dict[str, list]):
    for _, events in entries.items():
        format_string = '%Y/%m/%d'
        now = datetime.now(timezone.utc)
        date_key = now.strftime(format_string)
        s3_key = f'unified/carsales/{date_key}/{now.isoformat()}_{str(uuid4())}.json'

        df = convert_unified_df(events)
        if len(df) > 0:
            ...
            logger.info(f'Uploaded {len(df)} rows to {s3_key}')
        else:
            logger.info(f'No data uploaded')
