...
def transform_csv_to_entries(csv_content, received_datetime, s3_uri):
    ...
    for row in reader:
        ...
        for table, table_mapping in FIELD_MAPPINGS.items():
            entry[table] = {}
            for impel_field, autostock_field in table_mapping.items():
                if impel_field == "new_or_used":
                    entry[table][impel_field] = "New" if row.get(autostock_field, "") == "Y" else "Used"
                elif impel_field == "factory_certified":
                    entry[table][impel_field] = row.get(autostock_field, "") == "C"
                    ...
                    entry[table][impel_field] = row.get(autostock_field, None)
