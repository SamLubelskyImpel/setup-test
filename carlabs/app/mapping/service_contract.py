...
def map_service_contract(record: DataImports):
    ...
    if record.dataSource == 'CDK':
        ...
        service_contract.start_date = parsed_date(
            date_format, imported_data['ContractDate'][0])
        ...
        if isinstance(contracts, list) and len(contracts) != 0:
            ...
            service_contract.start_date = parsed_date(
                date_format, contracts[0]['ServiceContract']['ContractStartDate'])
            service_contract.deductible = contracts[0]['ServiceContract']['ContractDeductible']
            service_contract.expiration_months = contracts[0][
                'ServiceContract']['ContractExpirationMonths']
