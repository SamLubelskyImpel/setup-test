...
def map_sale(record: DataImports):
    ...
    if record.dataSource == 'CDK':
        date_format = '%Y-%m-%d'
        sale.sale_date = parsed_date(date_format, imported_data['DealEvent1Date'][0])
        sale.listed_price = imported_data['OutTheDoorPrice'][0]
        sale.sales_tax = imported_data['Tax1Amount'][0]
        sale.mileage_on_vehicle = parsed_int(imported_data['VehicleMileage'][0])
        ...
        sale.miles_per_year = int(imported_data['LeaseMileageAllowance'][0]) if imported_data['LeaseMileageAllowance'][0] else None
        sale.profit_on_sale = imported_data['FrontEndGrossProfit'][0]
        sale.vehicle_gross = imported_data['SalePriceWithWeOwes'][0]
        sale.delivery_date = parsed_date(date_format, imported_data['PickupDate1'][0])
        ...
        sale.mileage_on_vehicle = parsed_int(imported_data['vehicle']['Odometer'])
        ...
        sale.miles_per_year = parsed_int(imported_data['deal']['detail']['MilesYearActual'])
        sale.vehicle_gross = imported_data['deal']['detail']['RetailPrice']
        sale.delivery_date = parsed_date(date_format, imported_data['vehicle']['DateDelivered'])
        ...
        sale.date_of_inventory = parsed_date(date_format, imported_data['vehicle']['DateInInventory'])
        if imported_data['deal']['detail']['ServiceContracts']:
            sale.has_service_contract = len(imported_data['deal']['detail']['ServiceContracts']) != 0
    elif record.dataSource == 'DEALERVAULT':
        date_format = '%Y%m%d'
        sale.sale_date = parsed_date(date_format, imported_data['Contract Date'])
        ...
        sale.delivery_date = parsed_date(date_format, imported_data['Delivery Date'])
        sale.finance_rate = imported_data['APR']
        sale.finance_term = imported_data['Term']
        sale.finance_amount = imported_data['Amount Financed']
        sale.date_of_inventory = parsed_date(date_format, imported_data['Inventory Date'])
