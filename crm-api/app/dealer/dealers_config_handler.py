...
class DealersConfig:
    ...
    def _handle_post(self) -> dict:
        ...
        try:
            dealer_info = DealerInfo(**self.body)
            self.db_manager.create_filters(dealer_info.filter_dealer())
            return self.db_manager.post_dealers_config(dealer_info)
    ...
    def _method_not_allowed_response(self) -> dict:
        return {
            'statusCode': 405,
            'body': dumps({"error": "Method Not Allowed"})
        }
...
def lambda_handler(event: Any, context: Any) -> Any:
    ...
    try:
        ...
        return {
            "statusCode": 400,
            "body": dumps({"error": "Invalid request data."})
        }
...
if __name__ == '__main__':
    response = lambda_handler(event={
            ...
            "body": {
                ...
                "metadata": {
                    "userId": "TESTID",
                    "adf_email_recipients": ["konstantine@impel.test"]
                },
