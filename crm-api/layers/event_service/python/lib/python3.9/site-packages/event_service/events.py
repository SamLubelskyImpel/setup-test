...
def send_alert_notification(message, subject) -> None:
    ...
    sns_client.publish(
        ...
        Subject=f'INS: {subject}',
        MessageStructure='json'
    )


def dispatch_event(request_product: str, partner: str, event: Event, content: object, resource: Resource):
    event_type = f'shared-layer.crm.{"chatai" if request_product == "chat_ai" else "salesai"}.{resource.value}.{event.value}'
    event = {
    ...
    try:
        send_event('com.impel.shared-layer.crm', { 'events': [event] })
    except Exception as e:
        logger.exception(f'Failed to dispatch INS event {event}')
