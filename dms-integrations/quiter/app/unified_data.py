...
def validate_unified_df_columns(df):
    ...
    logger.info("Starting validation of unified DataFrame columns")
    ...
    logger.info("Validating if all DF columns exist in the database schema")
    ...
    logger.info("Validating if all database columns exist in the DF")
    missing_columns = []
    ...
    logger.info("Checking for empty columns in the DF")
    columns_with_no_data = df.columns[df.isna().all()].to_list()
    if columns_with_no_data:
        logger.warning(f"DF contains empty columns with no data: {columns_with_no_data}")

    logger.info("Validation of unified DataFrame columns completed")
...
def upload_unified_json(json_list, integration_type, source_s3_uri, dms_id):
    ...
    try:
        logger.info(
            f"Starting upload process for source: {source_s3_uri} and DMS ID: {dms_id}"
        )
        ...
        logger.info(
            f"Extracted date: Year={upload_year}, Month={upload_month}, Date={upload_date}"
        )

        df = convert_unified_df(json_list)
        logger.info(f"DataFrame created with {len(df)} rows from JSON list")
        ...
        if len(df) > 0:
            ...
            logger.info(f"Generated file name: {parquet_name}")
            ...
            logger.info(f"Uploading JSON to S3 at {s3_key}")
            s3_client.put_object(Bucket=INTEGRATIONS_BUCKET, Key=s3_key, Body=json_str)
            logger.info(
                f"Successfully uploaded {len(df)} rows for {source_s3_uri} to {s3_key}"
            )
        else:
            logger.info(f"No data to upload for {source_s3_uri}")

    except Exception as e:
        logger.exception(f"Error during upload process for {source_s3_uri}: {e}")
        raise
