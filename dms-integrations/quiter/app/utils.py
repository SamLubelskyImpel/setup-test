...
def merge_files(vehicle_sales_df, customers_df, vehicles_df):
    """
    Merge vehicle sales, customers, and vehicles dataframes on customer_id and VIN.
    
    The function performs an inner join between:
      - Vehicle Sales and Consumers on "Consumer ID" and "Dealer Customer No"
      - Vehicle Sales and Vehicles on "Vin No"
    
    After merging, redundant columns are removed.
    """
    try:
        # First, merge Vehicle Sales with Consumers based on Consumer ID (customer_id)
        merged_df = pd.merge(vehicle_sales_df, customers_df, left_on="Consumer ID", right_on="Dealer Customer No", how="inner")
