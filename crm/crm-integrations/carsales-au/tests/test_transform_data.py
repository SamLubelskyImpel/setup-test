...
class TestIntegration:
    ...
    def test_record_handler_success(
        ...
        mock_carsales_raw_data: Dict[str, Any],
        s3_client,
        secretsmanager_client,
        caplog: pytest.LogCaptureFixture
    ) -> None:
        """Test successful end-to-end record processing."""
        # Setup S3
        bucket_name = "test-bucket"
        s3_client.create_bucket(Bucket=bucket_name)
        s3_client.put_object(
            Bucket=bucket_name,
            Key="carsales/leads/DEALER_ABC/lead.json",
            Body=json.dumps(mock_carsales_raw_data)
        )

        # Setup Secrets Manager
        secret_data = {"impel": json.dumps({"api_key": "test-api-key"})}
        ...
        with caplog.at_level(logging.INFO):
            record_handler(mock_sqs_s3_event["Records"][0])

        # Verify
        assert "SUCCESSFULLY CREATED" in caplog.text.upper()
